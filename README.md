...
메뉴 버튼들도 바닐라 아이콘 대신 직접 그린 커스텀 아이콘을 사용합니다 (전부
`CustomModelData` 기반이라 리소스팩이 없으면 원래 바닐라 아이콘으로 자동 대체됩니다).

| 버튼 | 기본 아이템 | 아이콘 |
|---|---|---|
| 스폰 | 나침반 | 금빛 별 |
| 엔더상자 | 엔더상자 | 보라 보석이 박힌 상자 |
| 주식 (메인메뉴) | 에메랄드 | 상승 막대그래프 |
| 랜덤 TP | 엔더진주 | 소용돌이 포탈 |
| 대장간 강화 | 모루 | 망치+모루+불꽃 (레바테인과 같은 톤) |
| 홈 | 레드 베드 | 아기자기한 집 |
| 보유 포인트 (주식) | 금괴 | 동전 주머니 |
| 뒤로가기 (주식) | 화살 | 곡선 화살표 |
| 주식 종목 아이콘 | (종목마다 다른 바닐라 소재) | 초록 잎+동전 |
...
resource-pack-sha1=70041cd0e58f6e9c891d01c6b03677f51b3216c3
```
