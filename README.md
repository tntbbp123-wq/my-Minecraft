...
- `/serverselect` (별칭 `/서버선택`, `/worldselect`) — 서버/월드 선택 로비 메뉴.
  항목별로 이름·색상·태그·접속 인원·안내 문구를 보여주고, 클릭하면 해당 Multiverse 월드로
  이동합니다. 항목이 `page-size`(기본 7)개보다 많으면 좌/우 화살표로 페이지 이동.
  목록은 `config.yml`의 `worldselect.entries`에서 자유롭게 추가/수정 가능
  (`world` 값이 실제 존재하는 월드 이름과 일치해야 클릭 시 이동됩니다).
