...
- 주식(모의 투자) — 메뉴에서 가상 종목을 내부 포인트로 매수/매도. 가격은 **1시간마다** 갱신되며,
...
- `/뉴스작성 <종목명> <상승|하락> <변동폭%> <내용 또는 AI:주제>`,
  ...
  뉴스를 예약 작성. 작성 후 **1시간 뒤 전체 공지**로 공개되고, **2시간 뒤** (작성 시점 기준)
  진짜 뉴스라면 실제로 해당 종목 가격에 반영됨. 가짜 뉴스는 똑같이 공개되지만 가격에는
...
`/특수아이템소환`, `/주식종류추가`, `/뉴스작성`, `/가짜뉴스작성`, `/관리자메뉴`를 사용할 수 있습니다.
